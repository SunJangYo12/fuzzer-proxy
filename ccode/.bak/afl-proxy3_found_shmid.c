...
void afl_map_shm(struct Fstate *fstate)
{
  ...
  if (id_str)
  {
    ...
    int xshm_id = shmget(IPC_PRIVATE, __afl_map_size, IPC_CREAT | IPC_EXCL | 0644);
    ...
    syslog(LOG_INFO, "[*] commap_id = %s  %d", fstate->commap_id, xshm_id);


    __afl_area_ptr = shmat(xshm_id, NULL, 0);
    bzero((void *)__afl_area_ptr, __afl_map_size);
    ...
    syslog(LOG_INFO, "[*] afl_area_ptr[0] = %d", __afl_area_ptr[0]);
  }
}
